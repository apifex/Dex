...
const test = async () => {     
        ...
        await nft.methods.safeMint(accounts[0], 5, '5.json', 1000).send({from:accounts[0]})
        // await nftContract.mintMultiple(mintArrayAccounts, mintArrayIds, mintArrayJSONs);
        
        console.log('Token 0 metadata URI', await nft.methods.tokenUri(5).call())
        ...
        await nft.methods.approve('0xE705c20B5B7cEe760cCe1C86855848edfBF3cC9A' ,5).send({from: accounts[0]})
        // await nftContract.approve('0x254dffcd3277C0b1660F6d42EFbB754edaBAbC2B' ,2)

        console.log('owner of 0: ', await nft.methods.ownerOf(5).call())
        // console.log('owner of 1: ', await nft.ownerOf(1))
        // console.log('owner of 2: ', await nft.ownerOf(2))
        
        console.log('get approved 0: ', await nft.methods.getApproved(5).call())
        // console.log('get approved 1: ', await nft.getApproved(1))

        await dex.methods.makeAuctionOrder('0xc73076248c9bbf598b2ff0e5a3c8a576e41dd487', 5, 200, 10000).send({from:accounts[0]})

        console.log((await nft.methods.tokenUri(5).call()).toString())
